    ...
    while (!fileToParse->atEnd()) {
        ...
        if (line.contains("Standard orientation") ||
                ...
                "------------")) // i.e. not at the end of the table
            {
                ...
                atom.element =
                    periodicTable.value(list.at(1).toInt(testElement));
                atom.x = list.at(3).toFloat(testX);
                atom.y = list.at(4).toFloat(testY);
                atom.z = list.at(5).toFloat(testZ);
                if (testElement && testX && testY &&
                    testZ) { // Meaning all casts worked properly
        ...
        if (line.contains("Harmonic frequencies")) {
            harmonicFrequencies.clear(); // Remove all previous matches (calcall
                                         // computations for exemple)
            // Retrieve Three Lowest Harmonic Frequencies.
            // Start by reading the header but not save it
            for (int ctr = 1; ctr <= 3; ctr++) {
                fileToParse->readLine();
            }
            // Then actually save the next 6 lines that contains the Harmonic
            // Frequencies
        ...
        if (line.contains("SCF Done")) {
            ...
            hartreeFockEnergy =
                line.right(line.length() - equalPosition).trimmed();
            int AUposition = hartreeFockEnergy.indexOf("A.U.");
            hartreeFockEnergy =
                "HF " + hartreeFockEnergy.left(AUposition).trimmed();
        }
    }
    fileToParse->close();
